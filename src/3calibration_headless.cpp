/*
 * 3calibration_headless.cpp -- Calibrate 3 cameras in a horizontal line together (headless version for Jetson)
 */
...
static void help(char** argv)
{
        printf( "\nThis is a camera calibration sample that calibrates 3 horizontally placed cameras together.\n"
               ...
               "     [--no-display]           # run in headless mode without GUI\n"
...
{
    std::cout << "3-Camera Calibration (Headless Mode)" << std::endl;
    int i, k;
    int flags = 0;
    ...
    // Storage for detected corner points from all 3 cameras
    ...
    // Extract calibration parameters from command line
    boardSize.width = parser.get<int>("w");           // Number of inner corners horizontally
    boardSize.height = parser.get<int>("h");          // Number of inner corners vertically
    squareSize = parser.get<float>("s");              // Size of each square in real units
    aspectRatio = parser.get<float>("a");             // Camera aspect ratio (fx/fy)
    noDisplay = parser.has("no-display");             // Run in headless mode
    
    // Set calibration flags based on optional parameters
    if (parser.has("a"))
        flags |= CALIB_FIX_ASPECT_RATIO;              // Fix aspect ratio during calibration
    if (parser.has("zt"))
        flags |= CALIB_ZERO_TANGENT_DIST;             // Assume zero tangential distortion
    if (parser.has("p"))
        flags |= CALIB_FIX_PRINCIPAL_POINT;           // Fix principal point at image center
    
    outputFilename = parser.get<string>("o");         // Output file for calibration results
    inputFilename = parser.get<string>("@input");     // Input file listing all images
    ...
    // Read and validate input image list
    // Images must be grouped by 3 (one for each camera) and listed in order
    ...
    // Initialize camera matrices and distortion coefficients for all 3 cameras
    Mat cameraMatrix[3], distCoeffs[3], R[3], P[3], R12, T12;
    for( k = 0; k < 3; k++ )
    {
        cameraMatrix[k] = Mat_<double>::eye(3,3);     // 3x3 identity matrix
        cameraMatrix[k].at<double>(0,0) = aspectRatio; // Set initial fx based on aspect ratio
        cameraMatrix[k].at<double>(1,1) = 1;           // Set initial fy to 1
        distCoeffs[k] = Mat_<double>::zeros(5,1);       // Initialize distortion coefficients to zero
    }
    
    // Initialize rotation and translation matrices for camera pairs
    ...
    // Resize storage for corner points - one set per image per camera
    for( k = 0; k < 3; k++ )
        imgpt[k].resize(imageList.size()/3);

    // PHASE 1: CORNER DETECTION
    // Process each set of 3 images (one from each camera) to detect chessboard corners
    printf("Phase 1: Detecting chessboard corners...\n");
    int totalDetected = 0;
    for( i = 0; i < (int)(imageList.size()/3); i++ )
    {
        printf("Processing image set %d/%d:\n", i+1, (int)(imageList.size()/3));
        for( k = 0; k < 3; k++ )
        {
            // Map camera indices: input order -> internal order (0=right, 1=left, 2=center)
            int k1 = k == 0 ? 2 : k == 1 ? 0 : 1;
            printf("  Camera %d: %s", k+1, imageList[i*3+k].c_str());
    ...
    printf("\nTotal corners detected: %d/%d images\n", totalDetected, (int)imageList.size());
    printf("\nPhase 2: Running calibration...\n");

    bool calibrationSuccess = run3Calibration(imgpt[0], imgpt[1], imgpt[2], imageSize,
    ...
    printf("\nPhase 3: Saving calibration results to %s...\n", outputFilename.c_str());
    ...
    printf("\nPhase 4: Computing rectification transforms...\n");
    ...
    if (!noDisplay) {
        ...
        printf("\nCalibration completed successfully in headless mode.\n");
